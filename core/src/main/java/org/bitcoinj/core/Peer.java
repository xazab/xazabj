        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in Xazab Core hitting
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //
                        // Note that as of June 2012 Xazab Core won't actually ever interleave blocks pushed as
                        // part of chain download with newly announced blocks, so it should always be taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But Xazab Core may change in future so
